...
async function searchEvents() {
    ...
    try {
        const events = await fetchJson(`/searchEvents?query=${encodeURIComponent(query)}`);
        displayEvents(events);
    } catch (error) {
...
async function applyFilters() {
    ...
    try {
        const events = await fetchJson(
            `/filterEvents?title=${encodeURIComponent(title)}&location=${encodeURIComponent(location)}&date=${encodeURIComponent(date)}`
        );
        displayEvents(events);
    } catch (error) {
